...
A JavaFX plugin has been created in the project in order to properly package the application. The plugin:

- adds a `javafx` closure to projects using it ;
- adds a `copyBundleResources` task task copy the resources used for creating the bundle ;
- adds a `bundle` task creating the bundle itself.

The `javafx` closure allows:

- to define the bundle base name through the `bundleBaseName` property ;
- to specify the ant build script to use through the `antBuild` property ;
- to specify additional ant properties through the `antProperties` property ;
- to specify the bundle's resources through the `bundleResources` property.
...
* *js-object* : is the icon JavaScript object that will be used to callback to SlideshowFX ;
...
*** *icon* : the icon of the slide that will be displayed in SlideshowFX in the lst of available slide’s type ;
...
**** *attribute* : the icon of the attribute ;
**** *template-expression* : the icon of the template token. It is the Velocity token without the dollar sign ;
